				...
				$(".zxf_pagediv").createPage({
					...
					backfun: function(e) {
						...
						$.ajax({
							...
							success:function(data){
								// console.log(data);
								history.replaceState(null, null, "?pageind="+pageindex);
								$(".warting").addClass("active");
								...
								$(".endior").click(function(){
									var number = $(this).parents("td").parents("tr").index();
									// console.log(number);
									window.location.href = "prouncu-revise.html?postion="+data.list[number].postion+"&parameter_cn="+data.list[number].parameter_cn+"&describe="+data.list[number].describe+"&product_id="+data.list[number].product_id+"&pageind="+pageindex;
								})
