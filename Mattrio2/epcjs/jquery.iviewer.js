...
(function() {
    ...
    this.x = setter(function(val, skipCss) {
            ...
            if (!skipCss) {
                ...
                var x=this._x + (this._swapDimensions ? this.display_diff() / 2 : 0);
                $.each($('.box'),function (key,value) {
                    var scale_x=$(value).attr('alt').split(',')[0] * scale;
                    var imgLeft=scale_x + x;
                    $(value).css({
                        'left':imgLeft+ "px"
                    })
                })
    ...
    this.y = setter(function(val, skipCss) {
            ...
            if (!skipCss) {
                ...
                var y=this._y - (this._swapDimensions ? this.display_diff() / 2 : 0);
                $.each($('.box'),function (key,value) {
                    var scale_y=$(value).attr('alt').split(',')[1] * scale;
                    var imgTop=scale_y + y;
                    $(value).css({
                        'top':imgTop+ "px"
                    })
                })
    ...
    this.setImageProps = function(disp_w, disp_h, x, y, skip_animation, complete) {
        ...
        // var size=$('.cont img').width() / params.width
        var params = {
            width: w,
            height: h,
        ...
        var width=$('.cont img').attr('wid');
        scale=params.width / width;
        $.each($('.box'),function (key,value) {
            var scale_x=$(value).attr('alt').split(',')[0] * scale;
            var scale_y=$(value).attr('alt').split(',')[1] * scale;
            var imgLeft=scale_x + x + (this._swapDimensions ? this.display_diff() / 2 : 0);
            var imgTop=scale_y + y - (this._swapDimensions ? this.display_diff() / 2 : 0);
            var size=scale*30
            $(value).css({
                'width':size+"px",
                'height':size+"px",
