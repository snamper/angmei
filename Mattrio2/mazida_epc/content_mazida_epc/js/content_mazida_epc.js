...
$("#next").click(function(){
	...
	getEpcInfo(userid,nserepcend,nserepcstrt,nplblk,hmodtyp);
    window.history.replaceState(null,null,'?c='+nserepcend+'&d='+nserepcstrt+'&e='+nplblk+'&f='+hmodtyp+'&oe='+oe+'&epcnum='+num+'&choice='+choice+'&h='+name1+'&i='+name2+'&imgsrc='+cont[num].img.split("?")[0]+'&parameters='+parameters+'&localtion='+localtion)
})
...
function getEpcInfo(userid,nserepcend,nserepcstrt,nplblk,hmodtyp){
	$.ajax({
		type:"post",
		url:network+"/Mattrio/MazdaEpcInterface/getMazdaOes",
		data:{
			"userid":userid,
			"category_id3":nplblk,
...
//			})
			...
			$.each(data.points,function(key,value){
				...
				var span = $("<span style='position:absolute;top:"+(value.y*scale-3)+"px;left:"+(value.x*scale-6+parseInt($("#viewer2 img").css("margin-left")))+"px;width:"+widx+"px;height:"+heigy+"px;' class='span"+value.location+"'>");
