...
function demo(){
	$.ajax({
		...
		success:function(data){
            ...
            localStorage.setItem('ToyotaParameters',parameters)
            if(data.msg=='暂无该车型数据'){
				$("#loading").hide();
				alert("暂无数据")
				window.history.back();
...
function rightClick(data,name1,oe,index){
    $(".epc_a").click(function(){
        var ind = index?index:$(this).parents().index()
		var e = data.categorys2[ind].category_id2;
		var i = data.categorys2[ind].category_name2;
		var imgsrc=data.categorys2[ind].img;
		if(sessionStorage) {
		...
		$(this).attr("href","../content_fengtian_epc/content_fengtian_epc.html?a="+npl+"&b="+disc_no+"&c="+nserepcend+"&d="+nserepcstrt+"&e="+e+"&f="+hmodtyp+"&h="+name1+"&oe="+oe+"&epcnum="+$(this).parents(".contentname").index()+"&i="+i+"&imgsrc="+imgsrc+'&vin='+vin);
	});
}
		...
		success:function(data){
			...
			$(".contentul li").click(function(){
				...
				$(".epc_a").click(function(){
					...
					$(this).attr("href","../content_fengtian_epc/content_fengtian_epc.html?a="+npl+"&b="+disc_no+"&c="+nserepcend+"&d="+nserepcstrt+"&e="+e+"&f="+hmodtyp+"&h="+name1+"&oe="+oe+"&epcnum="+epcnum+"&i="+i+"&imgsrc="+imgsrc+'&vin='+vin);
				});
			})
		}
	})
