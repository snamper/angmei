...
function fn(){
	...
	$.ajax({
		url:network+"/MattrioManager/MnumManager/queryMnumMikey",
		type:"post",
		data:{
			"pageindex":pageindex,
			"query_str":query_str
...
$(document).on("click",".delthis",function(){
	...
	if(confirm("是否删除")){
		$.ajax({
			type:"post",
			url:network+"/MattrioManager/MnumManager/deleteMnumMikey",
			data:{
				"postion":postion
			},
			dataType:"json",
...
function datadel(){
	...
	if(confirm("是否删除")){
		$.ajax({
			type:"post",
			url:network+"/MattrioManager/MnumManager/deleteMnumMikeys",
			data:{
...
$(document).on("click",".endior",function(){
	...
	sessionStorage.setItem("pages",$(".current").html());
	window.location.href =  "mikey_hz_revise.html?postion="+$(this).parents("td").parents("tr").children().eq(1).html();
});
